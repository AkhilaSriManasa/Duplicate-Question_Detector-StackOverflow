...
data_list=[]
for i in range(1,21):
    site = requests.get('https://stackoverflow.com/questions/tagged/sql?page="+str(i)+"&sort=newest&pagesize=50');
    if site.status_code is 200:
        ...
        for question in questions:
            ...
            new_data = {"SQL": topic,"url": url, "body_content":body_content}
            data_list.append(new_data)
        with open ('tag_sql.csv','w') as file:
            writer = csv.DictWriter(file, fieldnames = ["SQL","url","body_content"])            
            writer.writeheader()
